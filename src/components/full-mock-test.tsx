...
export function FullMockTest() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!response.ok) { setRecorderState("ready"); setStatus("Mock Report 生成失败，请稍后重试。"); return; }
    ...
    <>
      <section className="grid gap-8 rounded-[36px] border border-black/8 bg-white px-6 py-8 shadow-[0_24px_80px_rgba(16,24,40,0.08)] lg:px-10">
        <div className="text-center">
          ...
          <h1 className="mt-6 text-4xl font-semibold tracking-[-0.06em] text-[#101828] sm:text-5xl lg:text-[4.8rem]">全真模考</h1>
        ...
        {activeIndex < 0 ? <div className="grid gap-6 rounded-[28px] border border-black/8 bg-[#fffcf6] p-8">
          ...
          {introStepIndex >= 0 ? <div className="mx-auto grid max-w-3xl gap-4 rounded-[24px] border border-[#8d7557]/18 bg-[#faf5eb] px-6 py-6 text-left"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8d7557]">Introduction</p><p className="mt-3 text-base leading-8 text-[#344054]">{examinerLine}</p></div>{introRemaining !== null ? <div className="rounded-[18px] bg-white px-4 py-3"><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#8d7557]">Answer Time</p><p className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#101828]">{formatDuration(introRemaining)}</p></div> : null}</div> : <div className="flex justify-center"><button type="button" onClick={startIntroduction} className="brand-button min-w-[240px]">开始模考</button></div>}
        </div> : null}

        {currentPrompt ? <div className="grid gap-8">
          <div className="grid gap-5"><div className="flex items-center justify-between gap-4"><div className="text-sm text-[#6f675c]">当前进度 {activeIndex + 1} / {session.prompts.length}</div><div className="text-sm text-[#6f675c]">{currentPrompt.part}</div></div><div className="h-3 overflow-hidden rounded-full bg-[#efe9dc]"><div className="h-full rounded-full bg-[#101828] transition-all" style={{ width: `${progressPercent}%` }} /></div></div>
          <div className="grid gap-4 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#8d7557]">{currentPrompt.topic}</p>
            <h2 className="mx-auto max-w-4xl text-3xl font-semibold tracking-[-0.05em] text-[#101828] sm:text-4xl">{currentPrompt.prompt}</h2>
            {examinerLine ? <div className="mx-auto max-w-3xl rounded-[24px] border border-[#8d7557]/18 bg-[#faf5eb] px-6 py-4 text-left"><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8d7557]">Examiner Lead-in</p><p className="mt-3 text-base leading-8 text-[#344054]">{examinerLine}</p></div> : null}
            {currentPrompt.cuePoints.length ? <div className="mx-auto max-w-3xl rounded-[28px] border border-black/8 bg-[#fffdf8] p-6 text-left"><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8d7557]">You should say</p><div className="mt-4 grid gap-2 text-base leading-8 text-[#344054]">{currentPrompt.cuePoints.map((point) => <p key={point}>- {point}</p>)}</div></div> : null}
            {prepRemaining !== null && isPart2Prompt(currentPrompt) ? <div className="mx-auto max-w-xl rounded-[24px] border border-[#8d7557]/18 bg-[#fff7ea] px-6 py-5"><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#8d7557]">Preparation Time</p><p className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-[#101828]">{formatDuration(prepRemaining)}</p><p className="mt-3 text-sm leading-7 text-[#5b5349]">倒计时结束后，考官会提示你开始回答。你可以在这段时间内整理 Part 2 的思路。</p></div> : null}
          </div>
          <div className="flex flex-wrap items-center justify-center gap-4">
            <button type="button" onClick={() => openPrompt(activeIndex)} disabled={["playing", "preparing", "recording", "processing", "submitting"].includes(recorderState)} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full border border-black/8 bg-white px-8 text-base font-medium text-[#101828] transition hover:border-black/12 hover:bg-[#f6f0e5] disabled:cursor-not-allowed disabled:text-[#98a2b3]"><Volume2 className="h-5 w-5" />Play Question</button>
            {recorderState === "recording" ? <button type="button" onClick={stopRecording} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#101828] px-8 text-base font-medium text-white transition hover:bg-[#1b2333]"><PauseCircle className="h-5 w-5" />Stop Recording</button> : recorderState === "processing" ? <button type="button" disabled className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#98a2b3] px-8 text-base font-medium text-white"><LoaderCircle className="h-5 w-5 animate-spin" />Saving Recording</button> : <button type="button" onClick={() => void startRecording()} disabled={["playing", "preparing", "submitting"].includes(recorderState)} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#101828] px-8 text-base font-medium text-white transition hover:bg-[#1b2333] disabled:cursor-not-allowed disabled:bg-[#98a2b3]"><Mic className="h-5 w-5" />Start Recording</button>}
            <button type="button" onClick={() => openPrompt(activeIndex)} disabled={["playing", "preparing", "recording", "processing", "submitting"].includes(recorderState)} className="inline-flex min-h-14 items-center justify-center rounded-full border border-black/8 bg-white px-8 text-base font-medium text-[#101828] transition hover:border-black/12 hover:bg-[#f6f0e5] disabled:cursor-not-allowed disabled:text-[#98a2b3]">Re-Answer</button>
            <button type="button" onClick={() => activeIndex < session.prompts.length - 1 ? openPrompt(activeIndex + 1) : setStatus("整套录音已完成，现在可以生成 Mock Report。") } disabled={["playing", "preparing", "recording", "processing", "submitting"].includes(recorderState) || !recordings[activeIndex]?.blob} className="inline-flex min-h-14 items-center justify-center rounded-full bg-[#101828] px-8 text-base font-medium text-white transition hover:bg-[#1b2333] disabled:cursor-not-allowed disabled:bg-[#98a2b3]">{activeIndex >= session.prompts.length - 1 ? "Finish Recording" : "Next Question"}</button>
          </div>
          <div className="grid gap-2 text-center text-sm text-[#6f675c]"><p>{status}</p><p>{usageLine}</p>{recorderState === "recording" ? <p>当前录音时长：{formatDuration(recordingElapsed)}</p> : null}<p>已完成 {completedCount} / {session.prompts.length} 题，总录音时长 {formatDuration(Math.round(totalDurationSeconds))}</p></div>
          ...
          {allRecorded ? <div className="grid justify-center gap-2 text-center"><button type="button" onClick={() => void handleSubmit()} disabled={recorderState === "submitting" || restoring} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full border border-black/8 bg-white px-8 text-base font-medium text-[#101828] transition hover:border-black/12 hover:bg-[#f6f0e5] disabled:cursor-not-allowed disabled:text-[#98a2b3]">{recorderState === "submitting" || restoring ? <><LoaderCircle className="h-5 w-5 animate-spin" />Generating Report</> : <><Play className="h-5 w-5" />Generate Mock Report</>}</button><p className="text-xs text-[#8d7557]">预计生成时间 1-2 分钟</p></div> : null}
        </div> : null}
      </section>
      {showUpgradePrompt ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#101828]/35 px-4"><div className="w-full max-w-lg rounded-[28px] bg-white p-7 shadow-[0_24px_80px_rgba(16,24,40,0.18)]"><h3 className="text-2xl font-semibold text-[#101828]">当前次数不足</h3><p className="mt-3 text-sm leading-7 text-[#475467]">Free 用户可以体验全真模考答题流程，但生成正式报告前需要先升级为 Pro 或 Ultra 方案。</p><div className="mt-6 flex gap-3"><button type="button" onClick={() => setShowUpgradePrompt(false)} className="inline-flex flex-1 items-center justify-center rounded-full border border-black/8 bg-white px-4 py-3 text-sm text-[#101828] transition hover:border-black/15 hover:bg-[#f7f1e7]">关闭</button><Link href="/me/pricing" className="inline-flex flex-1 items-center justify-center rounded-full bg-[#101828] px-4 py-3 text-sm font-medium text-white transition hover:bg-[#1b2333]">查看 AI口语定价</Link></div></div></div> : null}
    </>
  );
}
