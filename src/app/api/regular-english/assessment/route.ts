...
import { consumeRegularEnglishAnalysisCredit, ensureRegularEnglishUsage } from "@/lib/regular-english-account";
import { getRegularEnglishQuestionById } from "@/lib/regular-english-questions";
...
export async function POST(request: Request) {
  ...
  const question = getRegularEnglishQuestionById(questionId);
  if (!question) {
    return NextResponse.json<PracticeAssessmentApiResponse>(
      {
        ok: false,
  ...
  try {
      ...
      return NextResponse.json<PracticeAssessmentApiResponse>(
        {
          ok: false,
          code: "quota_exceeded",
          error: "本月口语素养 10 次免费分析已用完，将在下个月自动重置。",
          phases: {
    ...
    await consumeRegularEnglishAnalysisCredit(supabase, usage);

    await supabase.from("practice_attempts").insert({
      question_id: question.id,
      question_title: `${question.categoryTitle} / ${question.prompt}`,
      part: "Regular English",
